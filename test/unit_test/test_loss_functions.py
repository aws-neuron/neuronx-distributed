...
import torch
from commons import set_random_seed, print_separator, IdentityLayer
from neuronx_distributed.parallel_layers import parallel_state, scatter_to_tensor_model_parallel_region
...
def parse_args():
    ...
    parser.add_argument('--s3_bucket', default='s3://ktf-test-runs/neuronx_distributed_parallel_layers/loss_functions')
...
def test_parallel_cross_entropy(tensor_model_parallel_size):
    def torch_cross_entropy(
        batch_size: int, seq_length: int, vocab_size: int, logits_scale: float, seed: int, device, label_smoothing: float = 0.0
