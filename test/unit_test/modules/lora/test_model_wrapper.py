...
class TestModelWrapper(unittest.TestCase):
    def test_model_wrapper_single_device(self):
        try:
            ...
            assert type(lora_model.get_base_model()) == type(model)
        except:
    ...
    def test_model_wrapper(self, rank_mock):
        try:
            ...
            # assert isinstance(model.module, nxd.pipeline.NxDPPModel)
            # assert isinstance(model.module.original_torch_module, LoraModel)
            model_str = str(model)
    ...
    @patch("neuronx_distributed.utils.model_utils.get_local_world_size", MagicMock(return_value=32))
    @patch("torch.distributed.get_rank")
    def test_unified_model_wrapper(self, rank_mock):
        try:
            pipeline_cuts = [
                "transformer.h.1",
                "transformer.h.2",
            ...
            model = nxd.initialize_parallel_model(nxd_config, get_model)
            model = get_lora_model(model, get_lora_config())
    ...
    @patch("neuronx_distributed.utils.model_utils.get_local_world_size", MagicMock(return_value=32))
    @patch("torch.distributed.get_rank")
    def test_unified_model_wrapper_no_pp(self, rank_mock):
        try:
            nxd_config = nxd.neuronx_distributed_config(
                tensor_parallel_size=8,
                optimizer_config={
