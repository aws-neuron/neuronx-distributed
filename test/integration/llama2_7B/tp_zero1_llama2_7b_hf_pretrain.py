...
from common_utils.modeling_llama_nxd import LlamaForCausalLM
...
class Logger:
    def __init__(self, args, world_size, model_dtype):
        ...
        if args.gpu_event_file:
            data = load_events(args.gpu_event_file)
...
def train_llama(flags):
    ...
    while True:
        ...
        if global_step >= flags.steps_this_run:
            ...
            print("Done saving checkpoint", flush=True)
            optimizer.save_sharded_state_dict(flags.output_dir)
            print("Done saving optimizer", flush=True)
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="gpu event file for testing",
    )
    ...
    if os.environ.get("WORLD_SIZE"):
        if is_pjrt_device():
            os.environ.pop('NEURON_RT_ASYNC_EXEC_MAX_INFLIGHT_REQUESTS', None)
