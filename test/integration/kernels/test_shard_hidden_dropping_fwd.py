...
import json
from test_shard_hidden_dropping import moe_blockwise_fwd
import sys
import copy
...
if __name__ == "__main__":
    """
    Runs the Forward Integration test for the Shard Hidden Kernel
    Args:
    ...
    identifier = 'NKI_FWD_INTEGRATION_TEST'
    metadata = ""
    if len(metadata) != 0:
    ...
    for param in params:
      ...
      header = str(header)
      log_header = f"Hidden={H}, T={T}, Experts={E}, TopK={TOPK}, I_TP={I_TP}, BatchSize={BS}, dtype={dtype}, skip={dma_skip}, router={rtype}, capacityfactor={cf}, EP={EP}"
      ...
      if fail:
        ...
        logger.error(f"{log_header}, xfail Unable to allocate memory for goldens\n")
      else:
        try: 
          kpi_data = moe_blockwise_fwd(H, T, E, TOPK, I_TP, BS, dtype, dma_skip, rtype, cf, EP, 0)
