...
def _load_module_moe(cfg: ut.ExptCfg):
    return ut.initialize_neuron_model(cfg, move_to_device=False, transpose_shared_experts_weights=True).eval()
...
def _load_llama4_fp8_ckpt(ckpt_path: str):
    ...
    for weight_name, weight in layer_weights.items():
        new_weight_name = weight_name.replace("language_model.model.layers.1.feed_forward.router.", "router.linear_router.")
        new_weight_name = new_weight_name.replace("language_model.model.layers.1.feed_forward.experts.", "expert_mlps.mlp_op.")
        new_weight_name = new_weight_name.replace("language_model.model.layers.1.feed_forward.shared_expert.", "shared_experts.")
        new_weight_name = new_weight_name.replace("language_model.model.layers.1.post_attention_layernorm.", "rmsnorm.")
        new_weight_name = new_weight_name.replace("language_model.model.layers.1.", "")
        ckpt[new_weight_name] = weight.clone()
    for weight_name in set(ckpt.keys()):
        if "expert_mlps." in weight_name:
            ckpt[f"moe_fused_tkg.{weight_name}"] = ckpt[weight_name].clone()
    return ckpt

def _convert_to_cpu_ckpt(ckpt: dict):
    # do all the work in preshard hooks manually
    cpu_ckpt = {}
    for weight_name, weight in ckpt.items():
        if "shared_experts." in weight_name:
            cpu_ckpt[weight_name] = weight.clone().T
        else:
            cpu_ckpt[weight_name] = weight.clone()
    cpu_ckpt["router.weight_T"] = ckpt["router.linear_router.weight"].clone().T
    for weight_name in set(cpu_ckpt.keys()):
        if "rmsnorm." in weight_name:
            cpu_ckpt["moe_fused_tkg.post_attention_layernorm.weight"] = cpu_ckpt[weight_name].clone()
        elif "expert_mlps." not in weight_name:
            cpu_ckpt[f"moe_fused_tkg.{weight_name}"] = cpu_ckpt[weight_name].clone()
    dequant_ckpt = _dequantize_fp8_ckpt(cpu_ckpt)
    return dequant_ckpt
...
def _generate_random_fp8_weights(n_routed_experts, intermediate_size, hidden_size):
    ...
    checkpoint = {
        "rmsnorm.weight": torch.rand(hidden_size, dtype=torch.float32) * 2 - 1,
        "router.linear_router.weight": torch.rand(n_routed_experts, hidden_size, dtype=torch.float32) * 2 - 1,
        "expert_mlps.mlp_op.down_proj.weight" : down_weight,
        "expert_mlps.mlp_op.gate_up_proj.weight": fuse_weight,
        "expert_mlps.mlp_op.down_proj.scale": down_scale,
        "expert_mlps.mlp_op.gate_up_proj.scale": fuse_scale,
        "shared_experts.down_proj.weight": torch.rand(hidden_size, intermediate_size, dtype=torch.bfloat16) * 2 - 1,
        "shared_experts.gate_proj.weight": torch.rand(intermediate_size, hidden_size, dtype=torch.bfloat16) * 2 - 1,
        "shared_experts.up_proj.weight": torch.rand(intermediate_size, hidden_size, dtype=torch.bfloat16) * 2 - 1,
    }
    for weight_name in set(checkpoint.keys()):
        if "expert_mlps." in weight_name:
            checkpoint[f"moe_fused_tkg.{weight_name}"] = checkpoint[weight_name].clone()
...
class TestRoutedExpertsFP8(unittest.TestCase):
    ...
    def load_ckpt(self, cfg):
        if self.CKPT_PATH:
            neuron_ckpt = _load_llama4_fp8_ckpt(self.CKPT_PATH)
        else:
            neuron_ckpt = _generate_random_fp8_weights(
                cfg.num_experts,
                cfg.intermediate_size,
                cfg.hidden_size,
            )
        cpu_ckpt = _convert_to_cpu_ckpt(neuron_ckpt)
        return neuron_ckpt, cpu_ckpt
    ...
    def test_moe_quantized_flat_compiler_against_cpu(self):
        ...
        for test_config in _generate_test_configs():
            ...
            neuron_ckpt, cpu_ckpt = self.load_ckpt(test_config)
            # Initialize and execute CPU model
            init_cpu_env("nxd")
            parallel_layers.parallel_state.initialize_token_shuffle_group(1)
            test_config.quantized = False
            cpu_model = compile_cpu_moe_model(cpu_ckpt, _load_module_moe, test_config)
            ...
            del cpu_model, neuron_model, neuron_ckpt, cpu_ckpt
