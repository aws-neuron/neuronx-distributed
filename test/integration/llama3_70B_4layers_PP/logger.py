...
class Logger:
    def __init__(self, args, should_print=False):
        ...
        if (
            os.getenv("GOLDEN_EVENT_FILE", None) is not None
            and os.environ.get("NEURON_EXTRACT_GRAPHS_ONLY", None) is None
        ):
    ...
    def log(self, total_steps, loss, global_norm, current_lr, input_ids, throughput, start):
        ...
        if self.gpu_losses_to_compare is not None:
            if total_steps < len(self.gpu_losses_to_compare):
                if not torch.allclose(loss.cpu().float(), self.gpu_losses_to_compare[total_steps].float(), rtol=1.5e-1):
                    raise RuntimeError(
